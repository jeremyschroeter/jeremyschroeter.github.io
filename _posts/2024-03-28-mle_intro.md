...
In the image above, I plot the evolution of $50$ estimates of $P(H)$ as more coin flip landings are observed and used to update the MLE. Qualitatively we can tell our intuition that *"more data is better"* was also correct. As $n$ gets larger we see our estimates getting closer to the true value, albeit with most of the progress happening within the first $400$ observations. In fact, although we won't prove it here, under some basic assumptions it can be shown that in the limit as $n$ goes to infinity, the MLE estimate actually converges to the true value of $\theta$.
$$
\lim_{n\rightarrow \infty}\hat{\theta}_{\text{MLE}} = \theta
$$
